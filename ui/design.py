...
def help_screen(is_admin: bool = False) -> str:
    ...
    if is_admin:
        lines.extend([
            "",
            section("Админ", "Управление — кнопки меню: Опрос, Отчёт, Новости, Квиз, Топ-3, Рейтинги, Видео, Статистика, Ответ квиза."),
            "",
            ...
            "",
            section("Видео (YouTube + VK)", block([
                "/clearyoutube — сбросить список опубликованных YouTube-видео (повторная проверка)",
                "«🧹 Очистить предложения» — очистить очередь видео на модерации (YouTube + VK)",
            ])),
        ])
    return "\n".join(lines)
...
def start_screen(is_admin: bool = False) -> str:
    ...
    if is_admin:
        main.extend([
            "",
            section("Админ", "Используйте кнопки меню: Опрос, Отчёт, Новости, Квиз, Топ-3, Рейтинги, Видео, Статистика, Ответ квиза."),
        ])
    return "\n".join(main)
